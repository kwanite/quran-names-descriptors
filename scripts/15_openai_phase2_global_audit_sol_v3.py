...
def main() -> int:
    ...
    out_dir = run_dir / "phase2_global_audit_sol_v3"
    ...
    if MAX_OUTPUT_TOKENS != 128_000:
        raise RuntimeError(
            f"Internal configuration error: max_output_tokens="
            f"{MAX_OUTPUT_TOKENS}, expected 128000."
        )

    print("PHASE 2 GLOBAL AUDIT — PAID CALL PREFLIGHT")
    print("===========================================")
    print(f"Model: {MODEL}")
    ...
    print(f"Evidence: {out_dir.relative_to(PROJECT_ROOT)}")
    print()
    print("Calling GPT-5.6 Sol once for the global lexical-mapping audit...")
    ...
    if response_status == "incomplete":
        ...
        print()
        print("STOPPED: response was incomplete.")
        print("No automatic retry was performed.")
        return 3
    ...
    if not text:
        ...
        print()
        print("STOPPED: API response contained no output text.")
        print("No automatic retry was performed.")
        return 3

    print(f"Output text characters: {len(text)}")
    ...
    try:
        ...
        print()
        print("STOPPED: output text was not valid JSON.")
        print(str(exc))
        print("output_text.txt and raw_response.json were preserved.")
        print("No automatic retry was performed.")
        return 3
    ...
    if findings:
        ...
        print()
        print("STOPPED: global audit response failed deterministic validation.")
        print(json.dumps(findings, ensure_ascii=False, indent=2))
        print("No automatic retry was performed.")
        return 3

    print(f"Validated correction rows: {len(corrections)}")
